...
export async function GET() {
  try {
    ...
    // Filter for records with NextAction field that is:
    // AFTER the end of the current week (future dates)
    const futureRecords = transformedRecords.filter(record => {
      ...
      if (nextActionTimestamp && typeof nextActionTimestamp === 'number') {
        // Check if NextAction is AFTER the end of the week
    ...
    // Sort records by NextAction in ascending order (earliest future date first)
    ...
    console.error('Error fetching Future CRM data:', error);
    
    return NextResponse.json(
      { 
        success: false,
        error: 'Failed to fetch Future CRM data'
      },
      { status: 500 }
    );
  }
